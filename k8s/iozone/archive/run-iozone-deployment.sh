#!/bin/sh
#
# Filename: run-iozone-deployment.sh
#
# Run the IOzone metric container as a deployment
#
#set -x
...
IOZONE_POD_NAME="iozone"
CWD=${PWD}

# YAML files defining execution parameters of the container
IOZONE_DEPLOYMENT_YAML="${CWD}/iozone-deployment.yml"
IOZONE_DEPLOYMENT_YAML_TEMPLATE="${CWD}/iozone-deployment-template.yml"
...
cat "${IOZONE_DEPLOYMENT_YAML_TEMPLATE}" \
	...
	| sed "##/d" \
	> ${IOZONE_DEPLOYMENT_YAML}

# Start the IOzone test
kubectl create -f ${IOZONE_DEPLOYMENT_YAML}
