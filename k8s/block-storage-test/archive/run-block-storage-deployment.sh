#!/bin/sh
#
# Filename: run-block-storage-deployment.sh
#
# Run the Block Storage test container as a deployment
#
#set -x
...
BLOCK_STORAGE_POD_NAME="block-storage-test"
CWD=${PWD}

# YAML files defining execution parameters of the container
BLOCK_STORAGE_DEPLOYMENT_YAML="${CWD}/block-storage-deployment.yml"
BLOCK_STORAGE_DEPLOYMENT_YAML_TEMPLATE="${CWD}/block-storage-deployment-template.yml"
...
# Create the YAML for the test deployment
cat "${BLOCK_STORAGE_DEPLOYMENT_YAML_TEMPLATE}" \
	...
	| sed "##/d" \
	> ${BLOCK_STORAGE_DEPLOYMENT_YAML}

# Start the IOzone test
kubectl create -f ${BLOCK_STORAGE_DEPLOYMENT_YAML}
...
# Watch the block store log for results
